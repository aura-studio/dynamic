...
func (e *Env) GetOS() string {
	goos := strings.ToLower(strings.TrimSpace(e.getGoEnvVar("GOOS")))
	if goos == "" {
	...
	switch goos {
	case "linux":
		if desc := e.detectLinuxDescriptor(); desc != "" {
			return desc
		}
		return "linux"
	case "windows":
		if v := e.detectWindowsVersion(); v != "" {
			return "windows" + v
		}
		return "windows"
	case "darwin":
		if v := e.detectDarwinVersion(); v != "" {
			return "darwin" + v
		}
		return "darwin"
	default:
...
func (*Env) getGoEnvVar(key string) string {
...
func (*Env) detectLinuxDescriptor() string {
...
func (e *Env) detectWindowsVersion() string {
	...
	return e.extractFirstVersionLikeToken(string(out))
}

func (*Env) detectDarwinVersion() string {
...
func (*Env) extractFirstVersionLikeToken(s string) string {
...
func (*Env) getGoEnvArchVars() (goEnvArchVars, bool) {
